...
<!-- !for debugging what is not happening it can be useful to see dictionaries at various points try self.__dict__ -->
...
<span py:match="item.tag=='{http://id.webbrick.co.uk/}onoffEntry'" py:strip="" >
    <td py:if="javascript == 'yes'"
                ...
                height="${item.get('height')}"
                baseClassName="${item.get('baseClassName')}"
                wbLoad='loadWbOnOffEntry(&apos;${item.get("prefix")}&apos;,&apos;${item.get("format")}&apos;,&apos;${item.get("postfix")}&apos;)'
                py:content="getCurrentValue( item.get('wbSource') )"
    >Ignore</td>
    ...
    <div  py:if="javascript == 'yes'"
                ...
                >   
                    <div class='sliderValueContainer'>
                        <input id='num-input' class='sliderValue' />
                    </div>
