...
<p>These values are taken from the running system an may take several minutes to appear</p>
    ...
    <div py:for='pump in range(1,10,1)'>
        <span py:if='1'>
            <span py:if='(getCurrentValue("/eventstate/pump/%s/state" % pump))'>
                <h3>Status for pump ${pump}</h3>
                <p py:if='(getCurrentValue("/eventstate/pump/%s/state?attr=val" % pump))=="1"'>Status: Running </p>
                <p py:if='(getCurrentValue("/eventstate/pump/%s/state?attr=val" % pump))=="0"'>Status: Stopped </p>
            </span>
        </span>
    </div>
