...
class StateVariable(log.Loggable):
    ...
    def update(self, value):
        self.info("variable %s check for update %s (%s)", self.name, value, self.service)
        if not isinstance( self.service, service.Service):
            if self.name == 'ContainerUpdateIDs':
                ...
                if self.data_type == 'string':
                    ...
                    new_value = decode_boolean(value)
        ...
        if new_value == self.value:
            self.info("variable %s NOT updated, no value change %s", self.name, self.value)
            return
        ...
        self.warning("variable %s updated %s", self.name, self.value)
    ...
    def __repr__(self):
        return "Variable: %s, %s, %s, %s, %s, %s, %s, %s, %s, %s" % \
                        (self.name,
                         str(self.service),
                         self.instance,
                         self.implementation,
