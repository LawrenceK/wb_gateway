...
#    def __init__(self, control_point):
    def __init__(self, control_point, webserver):
        self.control_point = control_point
    
        global myurl
# If using global coherence object
#        myurl = 'http://%s:%d/events' % (control_point.coherence.hostname, control_point.coherence.web_server_port)
#        control_point.coherence.add_web_resource('events', self)

# if webserver separated from coherence
        myurl = webserver.add_web_resource('events', self)
        self.info("EventServer ready. %s", myurl)
...
# have service keep complete event URL
    ...
    def send_request(p, action):
        ...
        if action == 'subscribe':
            ...
            if timeout == 0:
                timeout = 1800
            request = ["SUBSCRIBE %s HTTP/1.1" % path,
                        "HOST: %s:%d" % (host, port),
                        "TIMEOUT: Second-%d" % timeout,
                        ]
            service.event_connection = p
        else:
            request = ["UNSUBSCRIBE %s HTTP/1.1" % path,
                        "HOST: %s:%d" % (host, port),
                        ]
    ...
    def got_error(failure, action):
        log.info(log_category, "error on %s request with %s", action,service.get_event_sub_url())
