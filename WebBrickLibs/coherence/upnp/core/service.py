...
# how logn before tiemout to attempt subscribe renewal
MAX_SUBSCRIBE_TIME  = 300
# how long before subscribe timesout we attempt renewal
SUBSCRIBE_WINDOW    = 60
# if a renew fails try again
SUBSCRIBE_RETRY_INTERVAL    = 20
...
class Service(log.Loggable):
    ...
    def __init__(self, service_type, service_id, url_base, control_url,
                 event_sub_url, presentation_url, scpd_url, device):

        self.service_type = service_type
        self.detection_completed = False
        self.id = service_id
        # set true if any state is evented
        self.does_sends_events = False

        self.control_url = urljoin(url_base, control_url)
        self.event_sub_url = urljoin(url_base, event_sub_url)
        self.presentation_url = urljoin(url_base, presentation_url)
        self.scpd_url = urljoin(url_base, scpd_url)
        ...
        self.renew_subscription_call = None

        self.client = None

        self.parse_actions()
        self.info("%s %s %s initialized" % (self.device.friendly_name,self.service_type,self.id))
        self.debug("control_url %s", self.control_url)
        self.debug("event_sub_url %s", self.event_sub_url)
        self.debug("presentation_url %s", self.presentation_url)
        self.debug("scpd_url %s", self.scpd_url)

    def __repr__(self):
        return "Service %s %s" % (self.service_type,self.id)

    def __str__(self):
        return "Service %s %s %s" % (self.service_type, self.control_url, self.id)
    ...
    def remove(self):
        ...
        try:
            if self.renew_subscription_call:
    ...
    def set_timeout(self, timeout):
        ...
        if timeout > MAX_SUBSCRIBE_TIME:
            timeout = MAX_SUBSCRIBE_TIME

        self.timeout = timeout
        if self.renew_subscription_call is not None:
            self.debug("Cancel existing renew subscription call for %s/%s",self.device.friendly_name,self.service_type)
            self.renew_subscription_call.cancel()
        
        self.renew_subscription_call = reactor.callLater(int(self.timeout)-SUBSCRIBE_WINDOW,
                                                             self.renew_subscription)
        self.info("starting renew subscription call for %s/%s to %d" % (self.device.friendly_name,self.service_type,int(self.timeout)-SUBSCRIBE_WINDOW))
...
#        return self.url_base
    ...
    def renew_subscription(self):
        ...
        self.renew_subscription_call = reactor.callLater(SUBSCRIBE_RETRY_INTERVAL, self.renew_subscription) # in case this attempt fails
        event.subscribe(self)
        ...
        def gotPage(x):
            ...
            for var_node in tree.findall('.//{%s}stateVariable' % ns):
                ...
                self._variables.get(instance)[name] = variable.StateVariable(self, name,
                                                               ...
                                                               data_type, values)
                if send_events.lower() == "yes":
                    self.does_sends_events = True
