...
class myHTTPPageGetter(client.HTTPPageGetter):
    ...
    def handleResponse(self, response):
        if self.quietLoss:
            return
        if self.failed:
            if self.headers.has_key('transfer-encoding') and self.headers['transfer-encoding'][0].lower() == 'chunked' and response:
                # this could still be chunked.
                # and there could still be a payload. A soap error has status 500 and more details in the body.
                response = de_chunk_payload( response )
