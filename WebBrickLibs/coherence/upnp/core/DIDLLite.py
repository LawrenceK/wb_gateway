...
class Object(log.Loggable):
    ...
    def fromElement(self, elt):
        ...
        for child in elt.getchildren():
            if child.tag.endswith('title') and not self.title:
                ...
                self.creator = child.text
            elif child.tag.endswith('streamContent'):
                self.title = child.text
#            else:
#                self.info("The child element is ignored %s - %s", child.tag, child.text)
