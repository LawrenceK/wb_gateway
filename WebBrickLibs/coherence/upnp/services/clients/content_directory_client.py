...
class ContentDirectoryClient(log.Loggable):
    ...
    def browse(self, object_id=0, browse_flag='BrowseDirectChildren',
        ...
        def got_process_result(result):
            ...
            # Shift to a list, so as to retain return order
            #r['items'] = {}
            r['items'] = []
            ...
            for item in elt.getItems():
                ...
                mdata = DIDLLite.DIDLElement()
                trck = DIDLLite.MusicTrack()
                trck.title = i['title']
                trck.description = i['title']
                trck.desc = i['title']
                trck.id = i['id']
                trck.parent_id = i['parent_id']
                mdata.addItem(trck)
                i['metadata'] = mdata.toString()
    ...
    def _failure(self, error):
        self.msg(error.getTraceback(), debug=True)
