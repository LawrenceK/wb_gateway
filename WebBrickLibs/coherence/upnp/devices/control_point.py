...
class ControlPoint(log.Loggable):
    ...
    def __init__(self, webserver, deviceList,auto_client=['MediaServer','MediaRenderer','BinaryLight','DimmableLight']):
        self.info("Create ControlPoint")
        self._deviceList = deviceList

        self.info("Coherence UPnP ControlPoint starting...")
        self.event_server = EventServer(self, webserver)
        webserver.add_web_resource('RPC2', XMLRPC(self))
    ...
    def browse(self, device):
        device = self._deviceList.get_device_with_usn(infos['USN'])
        if not device:
            return
        self.check_device( device)
    ...
    def get_devices(self):
        return self._deviceList.get_nonlocal_devices()

    def get_device_with_id(self, id):
        return self._deviceList.get_device_with_id(id)

    def get_device_by_host(self, host):
        return self._deviceList.get_device_by_host(host)

    def notify_device( self, client, device, isComplete=False ):
        #
        # notify users that a new device is available.
        # This also seperated so can override.
        #
        # isComplete is provided so can notify before initial data values received
        # Some devices do not send if device inactive.
        # may be we update library to explicitly retrive all state values.
        # for now allow isComplete test
#        if not isComplete:
        ...
        louie.send('Coherence.UPnP.ControlPoint.%s.detected' % client.device_type, None,
                               client=client,udn=client.device.get_id())
    ...
    def check_device( self, device):
        ...
        if device.client == None:
            ...
            if short_type in self.auto_client and not device.client:
                self.info("identified %s %r" % (short_type,device.get_friendly_name()))

                if short_type == 'MediaServer':
                    self.info("identified MediaServer %s", device.get_friendly_name())
...
#                self.notify_device( client, device, False )

        self.process_queries(device)

    def completed(self, client, udn):
	self.notify_device( client, client.device, True )

    def remove_client(self, udn, client):
        self.info("removed %s %s", client.device_type, client.device.get_friendly_name() )
        louie.send('Coherence.UPnP.ControlPoint.%s.removed' % client.device_type, None, client, udn=udn)
        client.remove()
    ...
    def put_resource(self, url, path):
        ...
        try:
            ...
            df = client.getPage(url, followRedirect=0, method="POST",
