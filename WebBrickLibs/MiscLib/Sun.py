...
class Sun:
    ...
    def sunRiseSet(self, lon=londonLog, lat=londonLat, year=None, month=None, day=None):
	"""
    ...
    def sunRiseSetDST(self, lon=londonLog, lat=londonLat, year=None, month=None, day=None ):
        """
        This computes times for sunrise/sunset.
        And then converts the times to daylight saving time.
        """
        now = time.localtime()
        if not (year and month and day):
            year = now[0]
        ...
        ss = self.__sunriset__(year, month, day, lon, lat, -35.0/60.0, 1)
        # Really we need to see whether dst applyes to year/month/day as it currently only works for today.
        if (now[8] == 1) and time.daylight :
            # DST defined and applies
            ss = ( ss[0]+time.daylight, ss[1]+time.daylight ) 
        return ss
...
if __name__ == "__main__":
    ...
    # to get your lat/long go to www.multimap.co.uk
    # enter your postcode
    # somewhere below the map will be listed lat and lon
    ...
    print "sunRiseSet today ", k.sunRiseSet()
    print "sunRiseSetDST today ", k.sunRiseSetDST()
    
    print "sunRiseSet ", k.sunRiseSet(londonLog, londonLat, yr,mn,dt)
    print "sunRiseSetDST ", k.sunRiseSetDST(londonLog, londonLat, yr,mn,dt)
    print "civilTwilight ", k.civilTwilight(yr,mn,dt, londonLog, londonLat)
    print "nauticalTwilight ", k.nauticalTwilight(yr,mn,dt, londonLog, londonLat)
    print "astronomicalTwilight ", k.astronomicalTwilight(yr,mn,dt, londonLog, londonLat)
