...
# $Id: DomHelpers.py 3573 2010-03-18 12:01:52Z andy.harris $
#

import types
from os.path import exists
from os import rename, remove, linesep, system
...
testFloat = re.compile( r'^-{0,1}\d+.\d*$' )
...
def saveXmlToFile(name, xmlDom, namebak=None, doBackup=True):
    """
    write the Xml DOM to a disk file without indentation or new lines.
    
    by default creates a backup file name+.'bak'
    """
    ...
    if exists(name+'.interim'):
        remove(name+'.interim')
    
    f = open(name+'.interim',"w")
    xmlDom.writexml(f)
    f.close()
    
    rename(name+'.interim', name)
    if platform != 'win32':
        system("sync")
    
def saveXmlToFilePretty(name, xmlDom, namebak=None, doBackup=True):
    """
    write the Xml DOM to a disk file with indentation and newlines.
...
def getDictFromXml(elm, typecast=False):
    """
    ...
    PS CHANGE: Added option to guess type of each value and convert it to int or float as required. Default behaviour is 
    to not guess the type and store everything as a string.  
    ...
    if nodes:
        result = list()
        for node in nodes:
            result.append( getDictFromXml(node, typecast) )
    else:
        result = dict()
            ...
            if typecast: 
                if testInt.match(txt):
                    try:
                        ...
                        result[""] = txt
                elif testFloat.match(txt):
                    try:
                        result[""] = round(float(txt),3)
                    except ValueError:
                        result[""] = txt
                    except TypeError:
                        result[""] = txt
        ...
        if elm.attributes:
            for idx in range(elm.attributes.length):
                ...
                # attempt convert to numeric
                if typecast:
                    if testInt.match(attr.value):
                        try:
                            result[attr.name] = int(attr.value)
                        except:
                            result[attr.name] = attr.value
                            
                    elif testFloat.match(attr.value):
                        try:
                            result[attr.name] = round(float(attr.value),3)
                        except:
                            result[attr.name] = attr.value
                    else:
                        result[attr.name] = attr.value
        ...
        for node in elm.childNodes:
            if isElement(node):
                newVal = getDictFromXml(node, typecast)
    ...
    if isinstance( obj, dict ):
        ...
        for k in obj:
            ...
            if k == '': 
                ...
                if k.endswith('s'):
                    rootelm = doc.createElement( k )    
                    for val in obj[k]:
                        child = doc.createElement( k[0:-1] )
                        rootelm.appendChild( child )
                        getXmlElemFromObject( doc , child, val )
                    addTo.appendChild(rootelm)
                else:
                    for val in obj[k]:
        ...
        rootName = addTo.nodeName
        if rootName.endswith('s'):    
                    for val in obj:
                        child = doc.createElement( rootName[0:-1] )
                        addTo.appendChild( child )
                        getXmlElemFromObject( doc , child, val )                   
        else:
                    for val in obj:
                        elm = doc.createElement( rootName )
                        addTo.appendChild( elm )
                        getXmlElemFromObject( doc, elm, val )    
