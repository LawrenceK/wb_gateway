...
<eventInterfaces>
    ...
    def testGetDictFromXml3(self):

        # Testing that type casting is working for int and float
        self.testdoc = ( """<?xml version="1.0" encoding="iso-8859-1" ?>
<root attr="attrtext">
  <integer1 value="12" />
  <integer2>
  ...
  <string2>
    #123456
        ...
        logging.debug( repr(self.testdoc) )
        dom = parseXmlString(self.testdoc)
        testDict = getDictFromXml( dom, typecast=True )
        logging.debug( repr(testDict) )
        self.assert_( testDict.has_key( "root" ) )
        rootDict = testDict["root"]
        self.assert_( rootDict.has_key( "integer1" ) )
        temp = rootDict["integer1"]["value"]
        self.assertEqual( temp, 12 )
        self.assert_( rootDict.has_key( "integer2" ) )
        temp = rootDict["integer2"][""]
        self.assertEqual( temp, 33 )
        self.assert_( rootDict.has_key( "float1" ) )
        temp = rootDict["float1"]["value"]
        self.assertEqual( temp, round(float("32.4"),3) )
        self.assert_( rootDict.has_key( "float2" ) )
        temp = rootDict["float2"][""]
        self.assertEqual( temp, round(float("0.5"),3) )
        self.assert_( rootDict.has_key( "string1" ) )
        temp = rootDict["string1"]["value"]
        self.assertEqual( temp, "testing" )
        self.assert_( rootDict.has_key( "string2" ) )
        temp = rootDict["string2"][""]
        self.assertEqual( temp, "#123456" )


    def testRegExps(self):
        #
        #  Test that the regular exoressions handle a variety of positive and negative numbers
        #
        self.assert_(testFloat.match("12.567"))
        self.assert_(testFloat.match("-12.567"))
        self.assert_(testInt.match("12"))
        self.assert_(testInt.match("-12"))
